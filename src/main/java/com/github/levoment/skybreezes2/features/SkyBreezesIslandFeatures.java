package com.github.levoment.skybreezes2.features;
...
public class SkyBreezesIslandFeatures extends Feature<DefaultFeatureConfig> {

    public SkyBreezesIslandFeatures(Codec<DefaultFeatureConfig> configCodec) {
        super(configCodec);
    }
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if (mainIsland) {
            ...
            // Plant a tree
            world.setBlockState(new BlockPos(startX + 8, 71, startZ + 8), Blocks.OAK_SAPLING.getDefaultState(), 3);
            BlockPos chestPosition = new BlockPos(startX + 2, 71, startZ + 8);
            // Place a chest
            world.setBlockState(chestPosition, Blocks.CHEST.getDefaultState().with(Properties.HORIZONTAL_FACING, Direction.NORTH).with(Properties.CHEST_TYPE, ChestType.SINGLE), 3);
            // Get the chest blockstate
            BlockState chestBlockState = world.getBlockState(chestPosition);
        ...
        if (secondIsland) {
            ...
            for (int y = 0; y < 8; y++) {
                for (int x = 0 + y; x < 17 - y - (y > 0 ? 1 : 0) ; x++) {
                    for (int z = 0 + y; z < 17 - y - (y > 0 ? 1 : 0) ; z++) {
                        if (y == 0) {
                            world.setBlockState(new BlockPos((startX + x), 70 - y, (startZ + z)), Blocks.GRASS_BLOCK.getDefaultState(), 3);
                        } else {
                            world.setBlockState(new BlockPos((startX + x), 70 - y, (startZ + z)), Blocks.DIRT.getDefaultState(), 3);
                        }
